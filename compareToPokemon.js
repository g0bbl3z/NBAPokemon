...
function compareToPokemon() {
  var compatiblePokemon = []

  userWeight = document.getElementById('userWeight').value;
  userHeight = document.getElementById('userHeight').value;

  adjustedWeight = (userWeight-60.3278)/.275;
  adjustedHeight = (userHeight-1.7)/.06713;
  while (i < 1007) {
    if (pokedex[i].heightm - adjustedHeight > -2) & (adjustedHeight - pokedex[i].heightm < 2) {
      if (pokemon[i].weightkg - adjustedWeight > -30) & (adjustedWeight - pokemon[i].weightkg < 30){
        list.push(pokemon[i].species);
        alert("pokemon added!");
      }
    }
    i++;
